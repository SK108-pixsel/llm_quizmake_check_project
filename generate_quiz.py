...
for target_answer in target_answers:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        # ★不合格だった場合、前回のフィードバックをプロンプトに足して書き直しを要求する
        ...
        print(f"問題文: {quiz_data['question']}")
        print(f"【スコア】 {eval_data['score']} / 10点")
        print(f"【理　由】 {eval_data['reason']}")
        
        # --- 3. 合格・不合格の判定 ---
        if eval_data["is_passed"]:
            print("=> ✨合格！CSVに保存します。")
            with open(csv_filename, mode='a', encoding='utf-8-sig', newline='') as f:
                writer = csv.writer(f)
                writer.writerow([quiz_data["answer"], quiz_data["question"]])
            break # 合格したので再試行ループを抜け、次のキーワードへ進む
            
        else:
            if attempt < MAX_RETRIES:
                print("=> ❌不合格。理由を元に再生成します...")
                # ★次のループに向けて、評価AIからの「ダメ出し」をセットする
